# Contributing to Codeagogo (Windows)
...
This project follows the [Contributor Covenant Code of Conduct](CODE_OF_CONDUCT.md). By participating, you are expected to uphold this code. Please report unacceptable behaviour via [GitHub Issues](https://github.com/aehrc/codeagogo-win/issues) or by contacting the project maintainers.
...
- WebView2 Runtime (for visualization features; included in Windows 11)
...
git clone https://github.com/YOUR_USERNAME/Codeagogo-win.git
cd Codeagogo-win
```
...
- All 5 hotkeys work correctly (Lookup, Search, Replace, ECL, Browser)
...
The project uses an `.editorconfig` file with `EnforceCodeStyleInBuild` enabled. Code style rules are enforced at build time. Ensure your editor supports `.editorconfig` to maintain consistency.
...
```csharp
/// <summary>
/// Looks up a concept by its identifier across configured code systems.
/// </summary>
/// <remarks>
...
public OntoserverClient(string baseUrl) { ... }
...
| Project | Description |
|---------|-------------|
| `Codeagogo.Tests` | Unit tests (ECL, search, validation, cache, models, etc.) |
...
| E2E Tests | Test full application workflows via FlaUI |
